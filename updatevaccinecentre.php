...
$username="SELECT * FROM tbl_userdetails WHERE userRole='VaccineCentre' AND userStatus=1";
...
<html>
    <head>
        ...
        <style>
            ...
            input[type=TEL],input[type=password],select,input[type="file"],input[type="number"]
            {
                width: 100%;
                padding: 12px 20px;
                margin: 8px 0;
...
if ($result=$conn->query($username))
{
    ...
    {?>
                ...
                <form action="vaccinecentreupdation.php?vetID=<?php echo $vetid ?>" method="POST" target="_self" enctype="multipart/form-data">
                   


                    <input type="tel" max=10 id="proname" value="<?php echo $vetphone ?>"size="30cm"pattern="^[0-9]*$" title="Must contain digits only" placeholder="Contact Number" name="vetphone" required><br>
                    <span id="lblError2" style="color: red;font-size:small"></span>
                   
                   
                    
                    <input type="submit" value="Save" name="updatevet" id="submitbutton">
        ...
        {
            echo "<br><br><table>";
            echo "<tr><td style='color:red'>VACCINE CENTRE INACTIVE</td></tr>";
            echo "<tr><td><marquee id='inactivevet' scrollamount='10'>Please activate VACCINE CENTRE PROFILE to continue</marquee></td></tr>";
            echo "<tr><td>Click <a href='vaccinecentredetails.php' style='color:blue'>here</a> to activate Vaccine Centre Profile</td></tr>";
            echo "</table>";
        }}?>
        	</div>
