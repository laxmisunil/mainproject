...
<div class="sidenav">
  ...
  <a href="vaccinecentredetails.php" id="active">Vaccination</a><br>
		...
		<main>
			<div class="head-title">
				<div class="left">
                    ...
                    <select name="month" style="padding:5px" required>
...
$yeardata="SELECT tbl1.*,sum(tbl2.productQuantity) AS proqua,tbl3.* FROM tbl_order AS tbl1 INNER JOIN tbL_orderitems AS tbl2 ON tbl1.orderID=tbl2.order_ID INNER JOIN tbl_productdetails AS tbl3 ON tbl2.productID=tbl3.productID WHERE tbl1.orderDate LIKE '$fulldate%' AND tbl1.orderStatus!=0 GROUP BY tbl1.orderDate ORDER BY tbl1.orderID DESC";
if ($result3=$conn->query($yeardata))
{
    if($result3->num_rows>0)
    {
        
        ?>
        <h3><font color="gray">Sales on <?php echo $monthname." ".$year ?></font></h3><br>
				...
				echo $monthinword." ".$day1.",".$year1; ?> 	</td>
                                <td><?php echo $row["proqua"]?>	</td>
                              
                                <td>
								<!--<div class="wrapper">
    								<a href="#demo-modal<?php //echo $orderdate?>" style="border:2px solid black;border-radius:4px;background-color:#060714;color:white;padding:4px;">View more</a>
								</div>
								<!--MODAL STARTS...-->

								<!--<div id="demo-modal<?php// echo $orderdate?>" class="modal">
 			...
 			?>	
                ...
                <?php 
                $proqua= $row["proqua"];
                $productssubtotal=$productssubtotal+$proqua;
                }
                   ...
                   ?>
                       ...
                       <tr><td></td><td><h3>Total Products</h3> </td><td><h3><?php echo $productssubtotal ?></h3></td></tr>
                       <?php
                       
$sel4="SELECT sum(productQuantity) AS totalcountorder FROM tbl_orderitems";
if($result4=$conn->query($sel4))
{
    if($result4->num_rows>0)
    {
        ...
        {
            $grandtotal=$row["totalcountorder"];

        }
       // $result4->free();
    }

$percentage=($productssubtotal/$grandtotal)*100;
echo "<font color='green'>".number_format((float)$percentage, 2, '.', '')."%</font> of Total Sales";
}
?>
 <br><br>
						   <form method="post" action="pdftodownload2.php?year=<?php echo $year?>&month=<?php echo $month?>">  
