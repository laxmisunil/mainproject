...
#plus:hover,#minus:hover
{
    ...
    <?php 
    $myorders="SELECT tbl1.*,tbl2.* FROM tbl_order AS tbl1 INNER JOIN tbl_orderitems AS tbl2 ON tbl2.order_ID=tbl1.orderID";
   ...
   <div style="display:flex">
    <table>
        <tr id="tr1">
            ...
            <th style="background-color:Gainsboro;text-align:center">Total Amount</th>
            <th style="background-color:Gainsboro;text-align:center">Ordered on</th>
  ...
  <td><img src='<?php echo $productImage ?>' width='40%' height='60%'></td>
             ...
             <td><?php echo $orderDate; ?></td>
                       <?php
                        ...
                        $productquan="SELECT * FROM tbl_cart WHERE productID='$productID' AND cartStatus=0 AND customerID='$userID'";
                        if($result=$conn->query($productquan))
                        {
                            if($result->num_rows>0)
                            {
                                while($row=$result->fetch_array())
