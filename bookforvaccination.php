    ...
    function ValidatePincode()
        {
            ...
            var expr2=/^(?!.*(\w)\1\1\1).*$/;
            if ((!expr.test(cuspincode))|(!expr2.test(cuspincode))) 
            {
            ...
            else if((expr.test(cuspincode))|(!expr2.test(cuspincode)))
            {
                lblError6.innerHTML = "";
                const button = document.querySelector('#submitbutton');
                button.disabled = false;
...
?>
                    ...
                    <input type="text"  placeholder="House/Apt No." id="housename" pattern="/^[a-zA-Z0-9 ]*$/" maxlength="30" name="housename" onkeyup="ValidateHouseName();" 
                    ...
                    <input type="text"  placeholder="Town" id="town" name="town" pattern="/^[a-zA-Z ]*$/" maxlength="30" onkeyup="ValidateTown();" 
                    value="<?php if ($userTown=="") echo ""; else echo $userTown?>"required><br>
                    <span id="lblError2" style="color: red;font-size:small"></span>
                  

                    <input type="text"  placeholder="ZIP/Postal Code" id="pincode" pattern="/([6]{1}[0-9]{5}|[1-9]{1}[0-9]{3}[0-9]{3})/" maxlength="6" name="pincode" onkeyup="ValidatePincode();" 
                    value="<?php if ($userPincode=="") echo ""; else echo $userPincode;?>" required><br>
                    <span id="lblError6" style="color: red;font-size:small"></span>
                 
                    

                    <input type="date"  placeholder="Date" id="date" name="date" onfocus="checklimit()" min="<?php echo date('Y-m-d', strtotime('+1 week')); ?>" max="<?php echo date('Y-m-d', strtotime('+1 month')); ?>" required>
                    <select id="time" name="time" required>
                    <option value="" disabled selected hidden>--Select Slot--</option>
                        <option value="9">Morning (09:30 am to 12:30 pm)</option>
                        <option value="2">Evening (02:30 pm to 05:30 pm)</option>
...
<script type="text/javascript">
    (function(d, m){
        var kommunicateSettings = 
            {"appId":"225da901aefd46b3d372afcc5bb6626fd","popupWidget":true,"automaticChatOpenOnNavigation":true};
