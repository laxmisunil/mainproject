...
if ($success === true)
{
    ...
    $orderinfo="INSERT INTO tbl_order VALUES (NULL,'$userID','$transactionID','$date','$time',1)";
    try
    {
                                    ...
                                    if ($result=$conn->query($sel))
                                    {
                                        if($result->num_rows>0)
                                        {
                                            while($row=$result->fetch_array())
                                            {
                                                ...
                                                $updatecart="SELECT productCount AS productCount3 FROM tbl_cart WHERE productID=$productID AND stockID=$stockID AND cartStatus=1 AND customerID!='$userID'";
                                                if($result11=$conn->query($updatecart))
                                                {
                                                    if($result11->num_rows>0)
                                                    {
                                                        while($row=$result11->fetch_array())
                                                        {
                                                            $productsincart=$row["productCount3"];

                                                            if($productsincart>$productcount)
                                                            {
                                                            $updatecartcount="UPDATE tbl_cart SET productCount=$productsincart-$productcount WHERE stockID=$stockID";
                                                            if($conn->query($updatecartcount)===true)
                                                            {
                                                                // header("location:home.php?status=1");
                                                            }
                                                            else 
