...
if ($success === true)
{
    ...
    if ($result=$conn->query($transactionid))
    {
        if($result->num_rows>0)
        { 
            while($row=$result->fetch_array())
    ...
    if ($result=$conn->query($selectorderid))
    {
        if($result->num_rows>0)
        {       
            while($row=$result->fetch_array())
            ...
            if ($result=$conn->query($products))
            {
                ...
                { 
                    ...
                    $productcount=$row["productCount"];
                    $stockID=$row["stockID"];
                    $selectprice="SELECT * FROM tbl_stock WHERE productID='$productID' AND stockID='$stockID'";
                    if ($result=$conn->query($selectprice))
                    {
                        if($result->num_rows>0)
                        { 
                            while($row=$result->fetch_array())
                            {
                                ...
                                $orderdetails="INSERT into tbl_orderitems VALUES(NULL,'$orderID','$productID','$stockID','$productcount',1)";
                                try
                                {
