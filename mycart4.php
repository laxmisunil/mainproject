    ...
    if ($result=$conn->query($sel))
{
   ...
   <?php 
   $sql1 = "SELECT tbl2.productStock,tbl1.productCount FROM tbl_cart AS tbl1 INNER JOIN tbl_stock AS tbl2 ON tbl1.stockID=tbl2.stockID WHERE tbl2.productStock>=1 AND tbl1.cartStatus=1 AND tbl1.customerID=$userID";
   $q1 = mysqli_query($conn,$sql1);
   $row = mysqli_fetch_array($q1);
   $countavailableproducts=$row["productCount"];

   $sql4 = "SELECT tbl1.productCount,tbl2.productPrice FROM tbl_cart AS tbl1 INNER JOIN tbl_stock AS tbl2 ON tbl1.stockID=tbl2.stockID WHERE tbl2.productStock>=1 AND tbl1.cartStatus=1 AND tbl1.customerID=$userID";
   $stotal=0;
   $totall=0;
   if($result8=$conn->query($sql4))
   {
       if($result8->num_rows>0)
       {
           while($row=$result8->fetch_array())
           {
            ...
            $procount=$row["productCount"];
         
            $stotal=$procount*$proprice;
            $totall=$totall+$stotal;
   ...
   $sql2 = "SELECT count(*) AS  countoutofstock,tbl2.productStock FROM tbl_cart AS tbl1 INNER JOIN tbl_stock AS tbl2 ON tbl1.stockID=tbl2.stockID WHERE tbl2.productStock=0 AND tbl1.cartStatus=1 AND tbl1.customerID=$userID";
    $q2 = mysqli_query($conn,$sql2);
    $row = mysqli_fetch_array($q2);

    $countrow=$row["countoutofstock"];

    $sql3 = "SELECT count(*) AS  countall,tbl2.productStock FROM tbl_cart AS tbl1 INNER JOIN tbl_stock AS tbl2 ON tbl1.stockID=tbl2.stockID WHERE tbl1.cartStatus=1 AND tbl1.customerID=$userID";
    $q3 = mysqli_query($conn,$sql3);
    $row = mysqli_fetch_array($q3);

    $countrow2=$row["countall"];

    if($countrow==$countrow2)
    {
...
?>
   ...
   <?php  ?>
   <br><br><br><h2><?php echo "Total Items :&nbsp&nbsp;".$countavailableproducts?></h2>
   <h2><?php echo "Subtotal&nbsp&nbsp:&nbsp₹".$subtotal ?></h2>
