...
?>
 <?php
      ...
      if((isset($_GET["status"])))
      {
          ...
          {
            ?>
          <div id="snackbar">Missing registered successfully!</div>
        ...
        else if(($_GET["status"])==3)
        {
          ...
          {
            ?>
             <div id="snackbar3">Post Deleted</div>
              <script> var x = document.getElementById("snackbar3");
            x.className = "show";
            setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000);</script>
...
$date=date('Y-m-d');?>
    ...
    <center><h1>Missing</h1></center>

<div style="display:flex">
&nbsp;
<button onclick="window.history.back();" id="showupcoming" style="margin-left:0.6cm"><i class="fa fa-arrow-left" aria-hidden="true" style="color:white"></i></button>&nbsp;
&nbsp;<button id="generateID" onclick="window.location.href='addmissing.php'">+ Add new missing case</button><br><br>

</div><br>

<?php
$sel="SELECT * FROM tbl_lostandfound WHERE lostandfoundStatus=1 ORDER BY missingPostedOn;";
...
<center>
    ...
    <?php 
    $i++;
    echo "<img src=".$row["animalImage"]." style='width=3%;height:220px' alt='missing'><br>";
    if(($row["animalName"])=="")
    {
        echo "<font size=5px>No Name</font><br>";
    }
    else
    {
        echo "<font size=5px><b>".$row["animalName"]."</font></b><br>";
    }
    if(($row["animalBreed"])=="NOT KNOWN")
    {
        echo $row["animalSpecies"]."-Breed not known<br>";
    }
    elseif(($row["animalBreed"])=="OTHERS")
    {
        echo $row["animalSpecies"]."-Breed not known<br>";
    }
    else
    {
        echo $row["animalSpecies"]."-".$row["animalBreed"]."<br>";
    }
  
    if(($row["beltColor"])=="No Collar")
    {
        echo "No collar worn<br>";
    }
    else
    {
        echo $row["beltColor"]." colored collar worn<br>";
    }

   
    echo $row["animalColor"]." Complexion<br>";
    $splitdate=explode("-",$row["missingFrom"]);
    $day=$splitdate[0];
    $month=$splitdate[1];
    $year=$splitdate[2];
    echo "Missing Since ".$year."-".$month."-".$day."<br>";
    echo "Last seen at ".$row["lastSeenAt"]."<br>";
    echo "<i class='fa fa-phone' aria-hidden='true'></i><b> ".$row["customerPhone"]."</b><br><i class='fa fa-envelope' aria-hidden='true'></i> ".$row["customerEmail"]."<br>";
    echo "<i class='fa fa-address-card' aria-hidden='true'></i> ".$row["customerAddress"].",<br>".$row["customerPincode"];
    if($posteduser!=$userID)
    {
     echo  "<br><br><font color='grey'>A handsome reward is promised!</font>";
    }
    else
    {
    ...
    {
        ?>
        <form action="deletemissingpost.php?lostandfoundid=<?php echo $row['lostandfoundID']?>&petID=<?php echo $petID ?>;" method="POST">
    <button id="generateID3" onclick="return confirm('Are you sure you want to delete this post?')"><i class="fa fa-trash" aria-hidden="true"></i> DELETE POST</button>
    </form>
    ...
    {
        ?>
         <form action="deletemissingpost.php?lostandfoundid=<?php echo $row['lostandfoundID'];?>" method="POST">
    <input type="hidden">
    </form>
    <?php
    }
...
{
    echo "<center><font size='5px'>No missing reports yet!</font></center>";
}
}
