...
$date=date('Y-m-d');?>
    ...
    <?php
$sel="SELECT tbl1.*,tbl2.*,tbl3.boosterRecommendation,tbl3.vaccineName FROM tbl_vaccinebooked AS tbl2 INNER JOIN tbl_pet AS tbl1 ON tbl1.petID=tbl2.petID INNER JOIN tbl_vaccinedetails AS tbl3 ON tbl2.vaccineName=tbl3.vaccineName WHERE tbl1.customerID='$userID' AND tbl2.vaccineDate<'$date' AND tbl2.vaccinebookedStatus=2 AND tbl1.petStatus=1";
if ($result=$conn->query($sel))
{?>
    <div class="div2" >
    <center><h4>Vaccination History</h4></center>
</div>
<div style="display:flex">
&nbsp;
...
&nbsp;<button onclick="window.location.href='myupcomingvaccinations.php'"id="showupcoming">Vaccinations to be done</button>&nbsp;
<button onclick="window.location.href=''"id="showupcoming"  style="background-color:Silver;color:black">Vaccination History</button>
</div><br>
<table>
<tr id="tr1">
    ...
    <th style="background-color:Silver">Vaccinated Date</th>
    <th style="background-color:Silver">Vaccinated Time</th>
    <th style="background-color:Silver">Next Vaccination on</th> 
   <!-- <th style="background-color:Silver">Status</th>-->   
    ...
    <?php } else?>
        ...
        <?php $_SESSION["vaccID"]=$row["vaccinebookedID"]?>
           ...
           ?>
            ...
            $duration=$row["boosterRecommendation"];
            
            if($duration==3)
            {
                $nextvaccination= date('Y-m-d', strtotime($vaccinationdate. ' + 1097 days'));
            }
            elseif($duration==2)
            {
                $nextvaccination= date('Y-m-d', strtotime($vaccinationdate. ' + 731 days'));
            }
            elseif($duration==1)
            {
                $nextvaccination= date('Y-m-d', strtotime($vaccinationdate. ' + 366 days'));
            }
            else
            {
                $nextvaccination= date('Y-m-d', strtotime($vaccinationdate. ' + 184 days'));
            }
            $nextvaccinationsplitted=explode("-",$nextvaccination);

                        $year=$nextvaccinationsplitted[0];
                        $month=$nextvaccinationsplitted[1];
                        $day=$nextvaccinationsplitted[2];
                        ...
                        $fulldate2= $monthinword." ".$day.",".$year;

            
            
            /*if($row["vaccinebookedStatus"]==2)
            {
   
                echo "<i class='fa fa-check' style='color:green' aria-hidden='true'></i>&nbsp;<font color='green'>Vaccination Done</font>";
            }
            ...
            */?>
            <td><b><font color="green"><?php echo $fulldate2; ?></font></b></td>
          
           
            <td><a href="deletevaccination.php?vaccinebookedID=<?php echo $row["vaccinebookedID"]; ?>" style="text-decoration:none;color:Red;" title="Delete from history"  id="generateID3" onclick="return confirm('Are you sure you want to delete this booking from history?');"><i class="fa fa-trash" aria-hidden="true"></i> DELETE </a></td>
    ...
    {
        echo "<tr><td colspan='7' style='color:gray'><center>You don't have any previous bookings.</td><tr>";
    }
}
else
{
