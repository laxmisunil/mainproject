...
?>
 <?php
      ...
      if((isset($_GET["status"])))
      {
          ...
          {
          ?>
          <div id="snackbar2">Appointment Cancelled Successfully!</div>
        ...
        {
          ?>
           <div id="snackbar3">Appointment Deleted Successfully!</div>
...
$date=date('Y-m-d');?>
    ...
    <?php
$sel="SELECT tbl2.*,tbl1.* FROM tbl_appointmentdetails AS tbl2 INNER JOIN tbl_pet AS tbl1 ON tbl2.petID=tbl1.petID AND tbl2.customerID=tbl1.customerID WHERE consultationDate<'$date' AND tbl2.appointmentStatus=0 ORDER BY consultationDate DESC";
...
<button onclick="window.location.href=''"id="showupcoming" style="background-color:Silver;color:black">Appointment History</button>
</div><br>
<table>
<tr id="tr1">
    ...
    <th style="background-color:Silver">Prescription</th>  
...
$petID=$row["petID"];
            ...
            if(($row["appointmentMedicine"]=="")AND($row["appointmentPrescription"]==""))
            {?>
                <td><font color="gray">No prescription added</font></td>
            <?php
            }
            else
            {
           ...
           <td><form action="prescription.php?appointmentID=<?php echo $row["appointmentID"];?>" method="POST"><br><input type="submit" id="generatepdf" value="Download"></form></td>
           <?php
            }
            ?> 
           <td><a href="deleteappointment.php?appointmentID=<?php echo $row["appointmentID"]; ?>" id="generateID3" style="text-decoration:none;color:red;" onclick="return confirm('Are you sure you want to remove this appointment from history?');"><i class="fa fa-trash" aria-hidden="true"></i> DELETE </a></td>
    ...
    {
        echo "<tr><td colspan='7' style='color:gray'><center>No previous appointments.</td><tr>";
    }
}
else
{
