...
$rowcheck="SELECT count(*) as countrows,productCount FROM tbl_cart WHERE productID=$productID AND stockID=$stockID AND customerID=$userID AND cartStatus=1";
...
if(mysqli_num_rows($result))
{
    ...
    $count = $row['countrows'];
    $productstockincart=$row["productCount"];
    
    //echo $count;

    if(($count>0)&&($productstockincart!=$productstock))
    {
        $new=$productcount+1;
    ...
    {
        header("location:home.php?status2=2");
    }     
  }
}
else
