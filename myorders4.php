...
if ($result=$conn->query($myorders))
{
   ...
   <div style="display:flex">
    <table>
    <tr id="tr1">
            <th style="background-color:Gainsboro">Order ID.</th>
       ...
       <?php
            ...
            $selproduct="SELECT tbl1.orderID,tbl1.orderDate,tbl2.order_ID,tbl2.productID,tbl2.stockID,sum(tbl2.productQuantity) AS proQuantity,tbl3.userID,tbl4.* FROM tbl_order AS tbl1 INNER JOIN tbl_orderitems AS tbl2 ON tbl1.orderID=tbl2.order_ID INNER JOIN tbl_userdetails AS tbl3 ON tbl3.userID=tbl1.customerID INNER JOIN tbl_orderitems AS tbl4 ON tbl4.order_ID=tbl1.orderID WHERE tbl1.customerID=$userID GROUP BY tbl4.order_ID";
            if($result2=$conn->query($selproduct))
            {
                if($result2->num_rows>0)
                {
                    while($row=$result2->fetch_array())
                    {
                        ...
                        <td><a href="myorders_onview.php?orderid=<?php echo $orderID; ?>" style="text-decoration:none">More Info</a></td>
